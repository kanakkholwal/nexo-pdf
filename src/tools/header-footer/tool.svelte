...
{:else}
  ...
  <div class="flex-1 overflow-y-auto bg-muted/10 p-6 space-y-8">
    <div class="max-w-4xl mx-auto space-y-8">
      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="space-y-2">
            ...
            />
            <p class="text-[10px] text-muted-foreground">Leave empty to apply to all pages.</p>
          </div>
          <div class="space-y-2">
          ...
          <div class="space-y-2">
            ...
            <div class="flex items-center gap-2">
                ...
                />
                <span class="text-sm font-mono uppercase text-muted-foreground">{store.state.fontColor}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="space-y-4">
        <h3 class="text-sm font-semibold border-b pb-2">Header Configuration</h3>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="space-y-2">
            <Label for="header-left">Header Left</Label>
            <Input id="header-left" type="text" placeholder={"{page} of {total}"} bind:value={store.state.headerLeft} />
          </div>
          <div class="space-y-2">
            <Label for="header-center">Header Center</Label>
            <Input id="header-center" type="text" placeholder="Document Title" bind:value={store.state.headerCenter} />
          </div>
          <div class="space-y-2">
            <Label for="header-right">Header Right</Label>
            <Input id="header-right" type="text" placeholder="Date" bind:value={store.state.headerRight} />
          </div>
        </div>
      </div>

      <div class="space-y-4">
        <h3 class="text-sm font-semibold border-b pb-2">Footer Configuration</h3>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="space-y-2">
            <Label for="footer-left">Footer Left</Label>
            <Input id="footer-left" type="text" placeholder="" bind:value={store.state.footerLeft} />
          </div>
          <div class="space-y-2">
            <Label for="footer-center">Footer Center</Label>
            <Input id="footer-center" type="text" placeholder={"- {page} -"} bind:value={store.state.footerCenter} />
          </div>
          <div class="space-y-2">
            <Label for="footer-right">Footer Right</Label>
            <Input id="footer-right" type="text" placeholder="" bind:value={store.state.footerRight} />
          </div>
        </div>
      </div>

      <div class="rounded-lg bg-blue-500/10 p-4 text-xs text-blue-600 dark:text-blue-400">
        <span class="font-semibold">Dynamic Variables:</span> Use <code>{'{page}'}</code> for the current page number and <code>{'{total}'}</code> for the total page count.
      </div>
