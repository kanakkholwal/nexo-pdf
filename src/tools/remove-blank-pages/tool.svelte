...
{:else}
  ...
  <div class="flex-1 overflow-y-auto bg-muted/10 p-6 space-y-8">
    <div class="max-w-4xl mx-auto space-y-8">
      ...
      <div class="rounded-xl border border-border bg-card p-6 shadow-sm space-y-6 max-w-xl mx-auto">
        ...
        <div class="space-y-4">
          <div class="space-y-2">
            ...
            <p class="text-[10px] text-muted-foreground">
                Higher sensitivity catches pages with tiny artifacts (like scan dust). Lower sensitivity only catches pure white pages.
            </p>
          </div>
          ...
          <Button 
            ...
            disabled={store.state.isDetecting || store.state.isProcessing}
          >
            {#if store.state.isDetecting}
                <Loader2 class="animate-spin mr-2" size={16} /> {store.state.progress}
            {:else}
          ...
          <div class="rounded-xl border border-border bg-card p-6 shadow-sm space-y-6">
            ...
            {#if store.state.detectedPages.length === 0}
                <div class="flex flex-col items-center justify-center py-8 text-muted-foreground">
                    ...
                    <p class="text-xs">Try increasing the sensitivity if you think some were missed.</p>
                </div>
            {:else}
                <div class="bg-blue-500/10 text-blue-600 dark:text-blue-400 p-3 rounded-lg text-sm mb-4">
                    Found <strong>{store.state.detectedPages.length}</strong> potentially blank page(s). Click on a thumbnail to unselect it if you want to keep it.
                </div>

                <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 bg-muted/30 p-4 rounded-lg border border-border max-h-100 overflow-y-auto">
  ...
  {#if store.state.hasPerformedDetection && store.state.detectedPages.length > 0}
      <div class="border-t border-border p-4 text-center">
        <Button
          ...
          disabled={store.state.isProcessing || store.state.detectedPages.filter(p => p.isSelected).length === 0}
        >
          {#if store.state.isProcessing}
            <Loader2 class="animate-spin mr-2" size={18} /> {store.state.progress}
          {:else}
